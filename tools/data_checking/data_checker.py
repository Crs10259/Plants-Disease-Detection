...
class ImageViewer(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Dataset Image Checker")
        ...
        self.mode_toggle_btn = QPushButton("Switch to Multi Mode")  # 默认显示切换到多图模式
        self.delete_btn = QPushButton("Delete Selected")
        self.renumber_btn = QPushButton("Renumber Selected")
        self.renumber_all_btn = QPushButton("Renumber All")
        ...
        self.empty_label = QLabel("Please select a directory with images to start")
    ...
    def select_directory(self):
        ...
        if dir_path:
            ...
            if not self.image_files:
                QMessageBox.warning(self, "Warning", "No images found in selected directory")
                return
    ...
    def show_single_image(self):
        ...
        # Add image counter
        counter_label = QLabel(f"Image {self.current_index + 1} of {len(self.image_files)}")
    ...
    def delete_selected(self):
        ...
        reply = QMessageBox.question(self, "Confirm Deletion", 
                                   f"Are you sure you want to delete {len(self.selected_images)} image(s)?",
    ...
    def renumber_selected(self):
        ...
        if not self.selected_images:
            QMessageBox.warning(self, "Warning", "No images selected for renumbering")
            return
        ...
        reply = QMessageBox.question(
            self,
            "Confirm Renumbering",
            f"Are you sure you want to renumber {len(selected_files)} image(s) starting from {start_number}?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            try:
                ...
                QMessageBox.information(
                    self,
                    "Success",
                    f"Successfully renumbered {len(selected_files)} image(s)"
                )
                ...
                QMessageBox.critical(
                    self,
                    "Error",
                    f"Error during renumbering: {str(e)}\nNo files were renamed."
                )
    
    def renumber_all(self):
        """重新排号所有图片"""
        if not self.image_files:
            QMessageBox.warning(self, "Warning", "No images in current directory")
            return
        ...
        reply = QMessageBox.question(
            self,
            "Confirm Renumbering All",
            f"Are you sure you want to renumber all {len(self.image_files)} images?\nThis will rename all images in sequential order.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            try:
                ...
                QMessageBox.information(
                    self,
                    "Success",
                    f"Successfully renumbered all {len(self.image_files)} images"
                )
